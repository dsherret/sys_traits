...
impl BaseFsMetadata for RealSys {
  ...
  fn base_fs_metadata(&self, path: &Path) -> Result<Self::Metadata> {
    fs::metadata(path).map(|inner| RealFsMetadata { inner })
  }

  #[inline]
  fn base_fs_symlink_metadata(&self, path: &Path) -> Result<Self::Metadata> {
    fs::symlink_metadata(path).map(|inner| RealFsMetadata { inner })
  }
}
...
impl FsDirEntry for RealFsDirEntry {
  ...
  fn metadata(&self) -> std::io::Result<Self::Metadata> {
    self.0.metadata().map(|inner| RealFsMetadata { inner })
  }
...
#[cfg(windows)]
fn stat_extra(file: &fs::File, fsstat: &mut WinStatInfo) -> Result<()> {
  use std::ffi::c_int;
